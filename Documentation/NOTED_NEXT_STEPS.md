...
- **Known follow-on:** on a *genuine* session expiry (refresh returns 401), the app still surfaces failed data calls rather than redirecting to the login screen. A clean "session expired → /login" redirect on definitive refresh failure is worth adding next.

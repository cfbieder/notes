...
- **[CR034](CR/CR034_drive_reconnect_startup_notice.md) — Google Drive reconnect notice at app startup.** Previously the "Reconnect required" prompt only lived inside the Settings → Google Drive card, so a user who never opened Settings had no signal that their Drive sync had silently paused on token expiry/revocation. Now `App.vue` fetches integration status on load and, if `needsReconnect` is true, shows a non-blocking **sticky warning toast** with a **Reconnect** action that routes to Settings; a persistent **amber dot on the Settings activity-rail icon** survives toast dismissal so the prompt stays discoverable. Both indicators auto-clear when a successful scan/reconnect flips `needsReconnect` back to false. Zero backend change — detection already existed via `GET /integrations/google-drive/status`. Touches [frontend/src/App.vue](../frontend/src/App.vue), [frontend/src/components/sidebar/ActivityRail.vue](../frontend/src/components/sidebar/ActivityRail.vue). See [§5.13 in NOTED_CURRENT_STATE.md](NOTED_CURRENT_STATE.md#513-google-drive-import-implemented).

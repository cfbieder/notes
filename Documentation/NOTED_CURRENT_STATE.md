...
- **Mobile drawer navigation:** On mobile, tapping the menu button opens `AppSidebar` as an overlay drawer. Picking a notebook or tag from the drawer closes it automatically (route-change watch in [NotesView.vue](frontend/src/views/NotesView.vue)) and renders [MobileNotesList.vue](frontend/src/components/mobile/MobileNotesList.vue) — a header-plus-list view scoped to the chosen notebook/tag. Tapping a note opens `MobileEditor`; back returns to the list. The `/notes` root keeps showing the `MobileHome` dashboard.

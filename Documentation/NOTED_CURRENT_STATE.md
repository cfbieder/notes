...
### 5.17 Encrypted Vault (CR020 + CR029, implemented)

Client-side, zero-knowledge vault for passwords, keys, credit cards, and bank accounts. Server stores opaque ciphertext only — the master password and derived key never leave the browser. Reachable via the sidebar (lock-key icon) or `/vault`.

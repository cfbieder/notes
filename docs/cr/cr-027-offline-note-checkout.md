...
| [`frontend/src/views/NotesView.vue`](frontend/src/views/NotesView.vue) (where the active editor mounts) | Render `<CheckoutBanner>` when the note has a checkout record. Add a note-menu item "Make available offline" / "Check in & keep offline" / "Check in & remove" / "Discard offline copy". |

...
- **Pluggable AI providers (CR038, in progress):** Migration `020_ai_provider_config.sql` adds the per-user `ai_provider_config` table (`UNIQUE(user_id, capability)`) for choosing an AI provider (`gateway`/`anthropic`/`openai`/`openai_compatible`) per capability. Provider API keys are stored only as AES-256-GCM ciphertext (`key_encrypted BYTEA`); the master key lives solely in the `AI_KEYS_ENC_KEY` env var (`openssl rand -hex 32`), never in the DB/backup. Foundation shipped (crypto util `src/utils/aiProviderCrypto.js`, repo `src/services/ai/providerConfig.js`, tests `cr038-*`); resolution falls back to the existing gateway/env path when a user has no row, so behavior is unchanged until configured. Phase 1 (text generation) is complete pending live-key verification: cloud adapters (`src/services/ai/adapters/` — anthropic native Messages API, openai + openai_compatible Chat Completions), SSRF-validated settings endpoints (`/api/v1/ai-providers`, `src/utils/ssrfGuard.js`), a Settings UI ("AI Provider (Text)"), and a provider-aware `/ai-assist/config` (`textProvider`). Phases 2 (OCR) and 3 (transcription) are not started.

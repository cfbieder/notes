...
### Released v0.16.3 (2026-08-30)

- **fix(editor): clicks land on the wrong line in notes containing horizontal rules.** In Normal mode, clicking a line placed the cursor one or more lines *below* it, and the drift grew further down the note — so only some notes looked broken. CodeMirror measures every line's height with `getBoundingClientRect()`, which **excludes margins**, so a vertical margin on a line decoration or block widget makes the real DOM taller than CodeMirror's height map; `posAtCoords()` then converts the click's y through the short map and resolves to a later line. `.cm-hr-line` carried `marginBottom: 8px`, so a note with 8 `---` rules drifted by up to 64px (~2.4 text lines) by the bottom. The table block widget (`margin: 10px 0`) and the PDF embed (`margin: 8px 0`) had the same latent bug. All three converted to padding, which is inside the measured box: `.cm-hr-line` `marginBottom` → `paddingTop` (total line height unchanged, so no visual shift), `.cm-table-wrapper` and `.cm-pdf-embed` `margin` → `padding` (the PDF border/radius moved onto the iframe). The inline image wrapper keeps its margin — it is `display: inline-block`, whose margin box *does* count toward line height. Source mode was unaffected (it skips the render plugins), which is the quickest way to confirm the diagnosis. ([sapphireTheme.js](frontend/src/lib/codemirror/sapphireTheme.js), [markdownRendering.js](frontend/src/lib/codemirror/markdownRendering.js))
- **chore: CI + mechanical convention guards.** [.github/workflows/ci.yml](.github/workflows/ci.yml) mirrors the real dev path — migrations replay against an empty Postgres, the API boots, the API suites run against it (`npm run test:ci`), and the frontend build runs as a separate job. [scripts/ci-guards.sh](scripts/ci-guards.sh) enforces the conventions that should block a merge rather than merely raise an eyebrow: no weak secret defaults in prod compose, forward-only and uniquely numbered migrations, no env files / `.claude/` / DB dumps tracked, and the native-dialog ratchet. Docs gained [cr-000-template.md](docs/cr/cr-000-template.md) as the starting point for new CRs and [docs/reviews/](docs/reviews/README.md) for dated review output that is still an active input (previously such reviews went straight to `archive/`).
...
- **fix(export): HTML notes now export to PDF with their own styling.** CR036's "Export as PDF" rendered HTML notes via `sanitizeNoteHtml()`, which extracts and discards every `<style>` block — so a themed HTML note (dark background, cards, timeline, colored badges) exported as flattened, unstyled markup that didn't match the on-screen render. Fixed in [printNote.js](frontend/src/lib/printNote.js): use `sanitizeNoteHtmlSplit()` with the same `.note-html` scope + container as `NotesView.vue`, re-inject the note's scoped CSS into the print `<head>`, add `print-color-adjust: exact` so backgrounds/colored graphics render in the PDF, and let HTML notes drive their own layout (no 800px markdown-column clamp). Markdown export unchanged. Verified: discarded-CSS path captured 0 rules; fixed path preserves all scoped rules; frontend build green. See [CR036 Post-release fixes](docs/cr/cr-036-export-note-as-pdf.md).

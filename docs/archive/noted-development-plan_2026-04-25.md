...
| 8.12 | ✅ AI Assist (desktop only) | Sidebar entry + `Cmd/Ctrl+Shift+A` shortcut open `AIAssistModal.vue`: prompt textarea + `NoteMultiPicker` (fuzzy search reuses `/notes?search=`) + live token gauge (warns at 85% of context window, red over limit). Backend `POST /api/v1/ai-assist/generate` concatenates selected note bodies, builds a prompt, and calls `llmService.generateText` → gateway `POST /llm/generate` (default model `qwen3:32b`, configurable via `LLM_GENERATION_MODEL`). Modal switches to a preview stage with editable title + body before save; Save creates a note via `POST /notes` with `is_ai_generated=true` and `ai_prompt` persisted. Sources are emitted as `[[wikilinks]]` in a `## Sources` section so backlinks/graph pick them up automatically. Migration `012_ai_generated_notes.sql` adds `is_ai_generated` and `ai_prompt` columns + a partial index. Hidden on mobile via `useMobile()`. Tests: `backend/tests/phase11-ai-assist.test.js` (34 assertions). |
| 8.12.1 | ✅ AI Assist enhancements (desktop only) | Five additions wired into the same modal/route: **(1) Streaming output** — `POST /generate` accepts `stream: true` and returns NDJSON via `reply.hijack()`; backend `llmService.generateTextStream` proxies the gateway's NDJSON token stream; frontend `aiAssistApi.generateStream()` reads `response.body` chunks via `ReadableStream` and appends to a live preview pane; `X-Accel-Buffering: no` to bypass nginx buffering. **(2) Model picker** — `GET /ai-assist/models` wraps gateway `/llm/models`; modal header dropdown lists available models; per-request `model` param overrides the default; choice persisted to localStorage (`noted.aiAssist.model`). **(3) Prompt history** — last 20 prompts saved to localStorage (`noted.aiAssist.history`), shown in a dropdown under the prompt textarea, click to refill. **(4) Condense toggle** — checkbox next to the token gauge; when on, backend pre-summarizes each note in parallel via `LLM_CONDENSE_MODEL` (default `phi4:14b`) using a 3-5 bullet prompt, then uses summaries as source blocks. **(5) Insert at cursor** — `NotesView.vue` registers the active `CodeMirrorEditor` exposed handle (`insertAtCursor`/`replaceRange`/`findRange`) into `aiAssistStore.editor` on mount; modal preview stage shows a second action button when an editor is registered, dropping the AI output at the cursor instead of creating a new note. Tests cover models endpoint, model param, condense flag, and streaming round-trip. |
...
- [x] ~~Focus mode / collapsible note list + context panels~~ — completed 2026-04-15. `uiStore` adds `noteListCollapsed` / `contextPanelsCollapsed` (persisted to localStorage) plus `toggleNoteList`, `toggleContextPanels`, `toggleFocusMode`, `toggleHelp`. `NotesView.vue` conditionally renders `NoteListPanel` and the `BacklinksPanel`/`LocalGraph`/`AttachmentZone` stack. `EditorToolbar.vue` gains two per-panel icon toggles (`PanelLeft`, `PanelBottom`) active only when a note is open. `AppSidebar.vue` footer adds a persistent Focus button (`Minimize2`/`Maximize2`, flips based on state) alongside Help and Sign out, and the footer now stacks actions on top of a username/version row (previously cramped on a single line). Keyboard: Alt+[ toggles note list, Alt+] toggles context panels, Alt+\ toggles focus mode (both panels), Alt+/ opens `HelpModal.vue` (new) listing all shortcuts. **Template bug fix:** `NotesView.vue` had a misplaced `<div v-else class="no-note">` attached to `TableEditorModal`'s `v-if` instead of `notesStore.currentNote`, causing the empty state to render underneath an active editor and steal the bottom half of the pane (most visible in focus mode). Moved the empty state to a proper `v-if="!notesStore.currentNote"` branch so `flex: 1` on the editor body reclaims full height.
